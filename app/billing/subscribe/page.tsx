          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            ...
            {PRICING_TIERS.map((tier) => {
              ...
              return (
                <div
                  key={tier.id}
                  data-testid={`billing-tier-card-${tier.id}`}
                  className={`relative flex flex-col rounded-2xl bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md ${
                    isPopular ? "border-2 border-registration-red" : "border border-line"
                  }`}
                >
                  {isPopular ? <BillingRecommendedBadge label="인기" /> : null}
                  ...
                  <button
                    ...
                    className="mt-6 w-full rounded-lg bg-registration-red px-4 py-3 text-sm font-semibold text-white transition-opacity disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {isSubmitting ? "결제창 여는 중…" : "지금 시작하기"}
                  </button>
                  <div className="my-6 border-t border-line" />
                  <div className="mt-auto">
                    <BillingPlanFeatureList tier={tier} />
                  </div>
                </div>
              );
            })}

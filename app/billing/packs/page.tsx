      ...
      <div className="flex min-h-screen flex-col bg-paper text-ink">
        ...
        <main className="mx-auto w-full max-w-3xl px-6 py-12">
          ...
          <div className="text-center">
            <p className="text-xs font-medium uppercase tracking-wide text-ink/40">PAGZLY 토큰</p>
            <h1 className="mt-2 text-3xl font-bold">토큰 팩 구매</h1>
          ...
          <p className="mt-4 text-center text-sm text-ink/65">
            <Link href="/billing/subscribe" className="font-medium text-registration-red hover:underline">
              정기 구독으로 더 저렴하게 받으려면 → 구독 보기
            </Link>
          </p>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2">
            ...
            {CREDIT_PACKS.map((pack) => {
              ...
              const isRecommended = pack.id === "pack_15";
              return (
                <div
                  key={pack.id}
                  data-testid={`billing-pack-card-${pack.id}`}
                  className={`relative flex flex-col rounded-2xl bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md ${
                    isRecommended ? "border-2 border-registration-red" : "border border-line"
                  }`}
                >
                  {isRecommended ? <BillingRecommendedBadge /> : null}
                  ...
                  <span className="mt-2 inline-flex rounded-full bg-line/30 px-2 py-0.5 text-xs text-ink/60">

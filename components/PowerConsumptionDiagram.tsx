...
export default function PowerConsumptionDiagram({
  ...
  const refs = selectNearbyPowerReferencePoints(w);
  if (refs.length === 0) return null;

  // 158차 소음/160차 방수/162차 무게 다이어그램과 동일 관례: 본문 위 가독성 위해
  // 카테고리 기본 accentText 우선.
  const stroke = category ? getCategoryTheme(category).accentText : theme.accentText;

  const allW = [...refs.map((r) => r.w), w];

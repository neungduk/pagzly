...
async function main() {
  ...
  const toolbarPath = path.join(SHOT_DIR, "75cha-canvas-toolbar-ai.png");
  await toolbar.screenshot({ path: toolbarPath });
  console.log(`[75cha] toolbar ${toolbarPath} (${bytes(toolbarPath)} bytes)`);

  const pendingLayer = split
    .locator('[data-testid="canvas-layer-panel"] button', { hasText: "AI 이미지 · 대기" })
    .first();
  await pendingLayer.click();
  await page.waitForTimeout(200);

  const aiPanel = split.locator('[data-testid="canvas-ai-image-panel"]');
  await aiPanel.waitFor({ state: "visible" });
  await freezeDetailScrollReveal(page);

  const panelPath = path.join(SHOT_DIR, "75cha-canvas-ai-image-panel.png");
  await aiPanel.screenshot({ path: panelPath });
  console.log(`[75cha] panel ${panelPath} (${bytes(panelPath)} bytes)`);

  const desktopCanvas = split.locator('[data-testid="canvas-frame"]');
  const canvasPath = path.join(SHOT_DIR, "75cha-canvas-ai-elements.png");
  await desktopCanvas.screenshot({ path: canvasPath });
  console.log(`[75cha] canvas ${canvasPath} (${bytes(canvasPath)} bytes)`);

  await browser.close();
  console.log("[75cha] canvas ai-image regression ✓");
}
